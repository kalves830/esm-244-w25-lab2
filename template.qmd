...
#choosing lowest AIC, model 2, as model of best fit
```
...
what metric?
  ...
  Let's make a function fro RMSE

for loop:
  apply the model to each training set
  make preiction on the test set with fitted training model
close loop:
...
```{r}
x=c(1,3,6,10,8,3,9,28,14,29)
y=c(19,56,72,81,44,53,62,90,80,62)
...
#can also use the following functions for randomized numbers
x1<-rnorm(10000)
y1<-rnorm(10000)
...
#without the replacement, the sample size will change when we resample -> replacement 
#puts the ball back into the pool
...
```{r}
#running this process again with random numbers every time does not allow for 
#reproducibility, which is why we use a seed

#take an interval -> set a seet
set.seed(42)
sample(ex, size=10, replace=TRUE)
...
#same number of rows as we do in the penguins dataset, how can we add in vector and mix it up
fold_vec<-rep(1:folds,length.out=nrow(penguins_clean))

#take from fold_vec the number of times we see the observations in penguins
...
Be able to do the fold once first, then we can do it many times

```{r}
#Example:
names <- c("Nate", "Kyle", "Sam")
...
#can add as many instructions as needed to loop
...
rmse_vec<-vector(mode="numeric", length=folds)  #Why? - we need to fill in what R will save, will make an empty vector, telling it what kind of vector, and how many times

for( i in 1:folds){

  # separate into test and train, i is referenced in both 4 loop and env -> error
  ...
  # Run for one model
  #basically copying from above functions with new kfold df
  ...
  rmse_vec[i]<-kfold_rmse$rmse_mdl #with the [] brackets, its going into the vector
}
...
#create new column, iterate over j, setting df and formula; doing for each kfold
